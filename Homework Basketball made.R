...
#FreeThrowAttempts per game code
myplot <- function(data, rows=1:10){
  Data <- data[rows,,drop=T]
  matplot(t(Data),type="b", pch=15:18,col=c(1:4,6))
  legend("topright",cex = 0.75,legend=Players[rows], col=c(1:4,6), pch=15:18, horiz=F)
}

myplot(FreeThrowAttempts/Games)

#Accuracy of Free Throws Code
myplot <- function(data, rows=1:10){
  Data <- data[rows,,drop=T]
  matplot(t(data),type="b", pch=15:18,col=c(1:4,6))
  legend("topright",cex = 0.75,legend=Players[rows], col=c(1:4,6), pch=15:18, horiz=F)
...
myplot((FreeThrows*100)/FreeThrowAttempts)

#Player Playiing Style
myplot <- function(data, rows=1:10){
  Data <- data[rows,,drop=T]
  matplot(t(data),type="b", pch=15:18,col=c(1:4,6))
  legend("topright",cex = 0.75, legend=Players[rows], col=c(1:4,6), pch=15:18, horiz=F)
